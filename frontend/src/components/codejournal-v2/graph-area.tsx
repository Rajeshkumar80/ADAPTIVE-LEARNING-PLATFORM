...
interface GraphAreaProps {
  ...
  onChangeType:     (id: string, newType: import("@/lib/codejournal-v2/content-types").ContentType) => void
  ...
  onMoveToWorkspace?: (blockId: string, workspaceId: string) => void
  onCopyToWorkspace?: (blockId: string, workspaceId: string) => void
  synthesisConnections?: Array<{ from: string; to: string; reason: string }>
}
...
function buildGraph(
  ...
  // Add edges from synthesizer connections (concept name matching)
  if (synthesisConnections.length > 0) {
    const blocksByText = new Map<string, string>()
    for (const b of blocks) {
      // Match by first few words of text (case insensitive)
      const key = b.text.toLowerCase().split(/\s+/).slice(0, 5).join(" ")
      blocksByText.set(key, b.id)
    }

    for (const conn of synthesisConnections) {
      const fromKey = conn.from.toLowerCase().split(/\s+/).slice(0, 5).join(" ")
      const toKey = conn.to.toLowerCase().split(/\s+/).slice(0, 5).join(" ")
      
      const fromId = blocksByText.get(fromKey)
      const toId = blocksByText.get(toKey)
      
      if (fromId && toId && blockSet.has(fromId) && blockSet.has(toId)) {
        const key = [fromId, toId].sort().join("§")
        if (!edgeSet.has(key)) {
          edgeSet.add(key)
          links.push({ source: fromId, target: toId, isSynthesisConnection: true, reason: conn.reason })
        }
      }
    }
  }
...
export function GraphArea({
  ...
  React.useEffect(() => {
    ...
    const { nodes, links } = buildGraph(blocks, ghostNote, cx, cy, nodesRef.current, deg, maxDeg, outerR, synthesisConnections)
          ...
          <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
            <div className="flex flex-col items-center gap-8 w-[420px]">
              ...
              <div className="flex flex-col gap-5 w-full">
                {([
                  { color: "var(--type-concept)",    label: "concept",    text: "Closures capture variables by reference, not value" },
                  { color: "var(--type-confusion)",  label: "confusion",  text: "Why does async/await not make code truly parallel?" },
                  { color: "var(--type-edgecase)",   label: "edgecase",   text: "Array.sort() mutates in place — caught me off guard" },
                  { color: "var(--type-hypothesis)", label: "hypothesis", text: "I think React re-renders children even if props don't change" },
          ...
          <g transform={`translate(${tx},${ty}) scale(${tk})`}>
            ...
            <g>
              {linksRef.current.map((link, i) => {
                ...
                const isSynthConn = (link as SimLink).isSynthesisConnection
                const dimmed = focalId != null &&
                  s.id !== focalId && t.id !== focalId
                const highlighted = focalId != null && !dimmed && !isSynth && !isSynthConn
                ...
                return (
                  <path
                    key={i}
                    d={d}
                    stroke={isSynthConn ? "#f97316" : "white"}
                    strokeWidth={isSynth ? 0.5 : highlighted ? 2 : isSynthConn ? 1.5 : 1.2}
                    strokeDasharray={isSynth ? "3 7" : undefined}
                    strokeOpacity={
                      isSynthConn ? (dimmed ? 0.05 : 0.4) :
                      dimmed      ? 0.02 :
                      highlighted ? 0.75 :
                      isSynth     ? 0.05 :
                      0.22
                  ...
                  >
                    {isSynthConn && (link as SimLink).reason && (
                      <title>{(link as SimLink).reason}</title>
                    )}
                  </path>
                )
              })}
                ...
                return (
                  ...
                  >
                    ...
                    <circle
                      r={r}
                      fill={fill}
                      fillOpacity={isSelected ? 1 : isHovered ? 0.97 : node.degree === 0 && !node.isSynthesis ? 0.4 : 0.90}
