...
export interface ConfidenceHistoryEntry {
  timestamp: number
  score: number
  reason: "enrichment" | "re-enrichment" | "resolved_boost" | "assignment_boost"
}
...
// Renders 5 small dots indicating confidence level (1–5)
export const ConfidenceDots = ({ level, accentColor }: { level: number; accentColor: string }) => (
    ...
    {Array.from({ length: 5 }, (_, i) => (
      <div
        ...
        style={{
          ...
          height: "6px",
          borderRadius: "50%",
          backgroundColor: i < level ? accentColor : "var(--muted-foreground)",
...
export const TileCard = memo(function TileCard({ 
  ...
  const config = CONTENT_TYPE_CONFIG[block.contentType] || CONTENT_TYPE_CONFIG.concept
  const Icon = config.icon
  const accent = config.accentVar
  const isTask = block.contentType === "assignment"
    ...
    <div
      ...
      style={{
        ...
        boxShadow: isTask
          ? "none"
          : block.contentType === "synthesis"
            ? `0 0 20px rgba(249, 115, 22, 0.1), inset 0 1px 0 0 rgba(255,255,255,0.05)`
            : block.isPinned
        ...
        opacity: block.confidence != null
          ? block.confidence <= 2 ? 0.6 : block.confidence === 3 ? 0.8 : 1.0
          : undefined
      }}
    >
      {/* Subtle radial glow */}
      <div
        ...
        style={{
          ...
          opacity: isTask ? 0.1 : block.contentType === "synthesis" ? 0 : block.isPinned ? 0.08 : isHighlighted ? 0.15 : 0.04,
        }}
      />
      ...
      >
        ...
        <div className={`flex items-center gap-2 flex-shrink-0 ${isTextRTL ? 'flex-row-reverse' : ''}`} style={{ color: "inherit" }}>
          ...
          {block.confidence != null && (
            <ConfidenceDots level={block.confidence} accentColor={accent} />
          )}
          {!effectiveCollapsed && block.contentType === "synthesis" && (
            <button
              onClick={(e) => {
                e.stopPropagation()
                  ...
                  <div className="w-full">
                    ...
                    <div className={block.isEnriching ? "shimmer-body" : ""}>
                      {isTask && block.subTasks ? (
                        ...
                        renderBody(block.text, undefined, accent)
                      )}
                    </div>
                  </div>
                )}
