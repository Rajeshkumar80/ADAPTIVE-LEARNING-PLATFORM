...
export function TilingMinimap({ pages, activePageIdx, onPageClick }: TilingMinimapProps) {
  ...
  return (
    <motion.div
      ...
      className="flex flex-col items-center gap-1.5 p-1.5 rounded-lg bg-black/60 backdrop-blur-md border border-gray-200 shadow-xl"
    >
      {pages.map((page, idx) => {
            ...
            <AnimatePresence>
              {hoveredIdx === idx && (
                <motion.div
                  ...
                  className="absolute right-full bottom-0 mr-2.5 w-52 p-2.5 rounded-md bg-black/92 backdrop-blur-md border border-gray-200 shadow-2xl pointer-events-none z-50"
                >
                  <p className="font-mono text-[7px] uppercase tracking-widest text-gray-400 pb-1.5 mb-1.5 border-b border-white/8">
                    ...
                    {page.map(block => {
                      ...
                      return (
                        <div key={block.id} className="flex items-start gap-1.5">
                          ...
                          />
                          <span className="font-mono text-[9px] text-gray-900/65 leading-snug">
                            {title}
                          </span>
                        </div>
                      )
            ...
            <button
              ...
              className={`group relative flex flex-col items-center gap-[4px] p-1.5 rounded-md transition-all duration-150 outline-none ${
                isActive
                  ...
                  : "border border-gray-200 bg-gray-50 hover:bg-white/[0.09] hover:border-white/25"
              }`}
            >
              ...
              <span className={`font-mono text-[7px] font-bold leading-none transition-colors ${
                isActive ? "text-primary/80" : "text-gray-900/35 group-hover:text-gray-500"
              }`}>
                {idx + 1}
              </span>
            </button>
