...
import { TileCard, type TextBlock } from "@/components/codejournal-v2/tile-card"
import { CONTENT_TYPE_CONFIG, type ContentType } from "@/lib/codejournal-v2/content-types"
import { getRelatedIds, useModKey } from "@/lib/codejournal-v2/utils"
...
export function TilingArea({
  ...
  const chunkedPages = useMemo(() => {
    const gridBlocks = blocks
      .filter((b: TextBlock) => b.contentType !== "assignment")
      .sort((a, b) => {
  ...
  const taskBlock = useMemo(() => blocks.find((b: TextBlock) => b.contentType === "assignment"), [blocks])
            ...
            {pageTrees.map((tree, idx) => {
              ...
              return (
                <div
                  key={idx}
                  data-page-idx={idx}
                  className={`flex w-full ${heightClass} border-b border-gray-100 last:border-0`}
                >
      ...
      {pageTrees.length === 0 && !taskBlock && (
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
          <div className="flex flex-col items-center gap-8 w-[420px]">
            <p className="font-mono text-[10px] uppercase tracking-[0.35em] text-foreground/35">K N O W L E D G E  C A N V A S</p>
