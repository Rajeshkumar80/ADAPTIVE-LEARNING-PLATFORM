...
import { TextBlock } from "@/components/codejournal-v2/tile-card"
...
export function TileIndex({ blocks, onHighlight, highlightedId, onClose, isOpen, viewMode }: TileIndexProps) {
  const getIcon = (type: string) => {
    switch (type) {
      case "assignment": return <CheckSquare className="h-3 w-3 text-indigo-400" />
      case "synthesis": return <Sparkles className="h-3 w-3 text-yellow-400" />
      case "confusion": return <HelpCircle className="h-3 w-3 text-blue-400" />
  ...
  const kanbanColumns = useMemo(() => {
    ...
    blocks.forEach(b => {
      ...
      else if (b.contentType === "assignment") cols.add("assignment")
      else cols.add(b.contentType || "concept")
    })
    return Array.from(cols).map(type => {
      const config = CONTENT_TYPE_CONFIG[type as ContentType] || CONTENT_TYPE_CONFIG.concept
            ...
            <div className="space-y-1">
               ...
               {kanbanColumns.map(col => (
                 <button
                    ...
                    className="flex items-center gap-2.5 w-full px-2.5 py-2.5 rounded-sm transition-all hover:bg-gray-50 text-left text-foreground/60 hover:text-foreground group"
                 >
              ...
              <div key={cat} className="space-y-1">
                ...
                {catBlocks.map(block => (
                   <button
                    ...
                    className={`flex items-start gap-2.5 w-full px-2.5 py-2 rounded-sm transition-all hover:bg-gray-50 text-left group border-r-2 ${
