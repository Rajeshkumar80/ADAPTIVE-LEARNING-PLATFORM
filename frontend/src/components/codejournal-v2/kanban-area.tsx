...
export function KanbanArea({
  ...
  const columns = useMemo(() => {
    const cols: Record<string, { title: string; icon: any; blocks: TextBlock[] }> = {
      ...
      assignment: { title: "Assignments", icon: CheckSquare, blocks: [] },
      synthesis: { title: "Synthesis", icon: Sparkles, blocks: [] }
    }
    ...
    blocks.forEach(block => {
      if (block.isEnriching) {
        ...
        const type = block.contentType || "concept"
        if (!cols[type]) {
    ...
    return Object.entries(cols)
      ...
      .sort(([keyA], [keyB]) => {
        // Priority: assignment > synthesis > processing > rest
        const order = ["assignment", "synthesis", "processing"]
        ...
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
          <div className="flex flex-col items-center gap-8 w-[420px]">
            ...
            <div className="flex flex-col gap-5 w-full">
              {([
                { color: "var(--type-assignment)", label: "assignment", hint: "Implement a debounce function from scratch" },
                { color: "var(--type-concept)",    label: "concept",    hint: "Closures capture variables by reference" },
                { color: "var(--type-confusion)",  label: "confusion",  hint: "Why does useEffect run twice in dev mode?" },
