...
export function KanbanMinimap({ columns, onColumnClick }: KanbanMinimapProps) {
  ...
  return (
    <div className="flex items-center gap-1.5 p-1 rounded-lg bg-gray-50 backdrop-blur-md border border-gray-100 shadow-xl transition-all hover:bg-gray-100">
      {columns.map((col) => (
        <button
          key={col.id}
          onClick={() => onColumnClick(col.id)}
          className="group relative flex items-center justify-center h-8 w-8 rounded-md hover:bg-gray-100 transition-all active:scale-95"
          title={col.title}
        >
