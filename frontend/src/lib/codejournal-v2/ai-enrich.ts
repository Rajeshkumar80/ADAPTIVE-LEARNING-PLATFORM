...
import { loadAIConfig, getBaseUrl, getProviderHeaders, getModelsForProvider } from "@/lib/codejournal-v2/ai-settings"
...
const SYSTEM_PROMPT = `You are a programming mentor embedded in a learning journal called CodeJournal.

## Your Job
Classify the student's journal entry into one of the programming-specific types and add a concise annotation that surfaces something the student didn't explicitly state — a related concept, a common pitfall, a deeper implication, or a connection to another programming topic.
...
- **No URLs or hyperlinks ever.** If you reference a source, use its name and author only. Never generate or guess a URL.
- Use markdown sparingly: **bold** for key terms, *italic* for titles. No bullet lists in annotations.
- Focus on programming insights: common bugs, performance implications, related patterns, edge cases the student might miss.

## Types
concept · confusion · edgecase · synthesis · conflict · assignment · resolved · definition · hypothesis

## Classification Guide
- **concept**: Student learned or explained a programming concept
- **confusion**: Student is confused or uncertain about something
- **edgecase**: Student discovered a boundary condition or special case
- **synthesis**: Student connected multiple concepts together
- **conflict**: Student found contradictory information or mental model clash
- **assignment**: A task or challenge (self-assigned or generated)
- **resolved**: Student resolved a previous confusion or conflict
- **definition**: Student defined a term or pattern
- **hypothesis**: Student proposed an untested theory about how something works

## Classification Priority
Use the most specific type. Default to 'concept' if nothing else fits clearly.
...
## Confidence Scoring
You MUST return a confidence score (1–5) in the "confidence" field of your JSON response. This score reflects how well the student understands the concept based on their writing quality. Use this rubric:

- **1** = Vague or incorrect terminology, no specifics, demonstrates minimal understanding
- **2** = Partially correct but shallow explanation, missing key aspects of the concept
- **3** = Correct but surface-level, no edge cases or deeper implications mentioned
- **4** = Clear explanation with correct terminology, shows some nuance and awareness
- **5** = Deep understanding, identifies edge cases, explains trade-offs, connects to related concepts

Evaluate based on these Writing Quality Signals:
- Clarity of explanation
- Correct use of terminology
- Identification of edge cases or boundary conditions
- Depth of reasoning and ability to connect ideas

Always return an integer 1–5. Never return null unless the entry is completely unrelated to a technical concept (e.g. a bare URL with no commentary).
...
export async function enrichBlockClient(
  ...
  forcedType?: string,
  category?: string,
  categoryConfidenceHistory?: { category: string; avgConfidence: number; count: number }[],
...
You have live web access. For this note type, include 1–2 real source citations by name, publication, and year. Do NOT generate URLs — reference by title and author only (e.g. "Per *Science*, 2023, Doe et al."). Only cite sources you have actually retrieved.`
  ...
  // URL prefetch (concept type with URL) — still server-assisted for CORS bypass
  let urlContext = ""
  const isUrl = /^https?:\/\//i.test(text.trim())
  if (effectiveType === "concept" && isUrl) {
  ...
  const confidenceHistoryContext =
    categoryConfidenceHistory && categoryConfidenceHistory.length > 0
      ? `\n\n## Category Confidence Context\n${categoryConfidenceHistory.map(
          (h) => `- ${h.category}: avg confidence ${h.avgConfidence.toFixed(1)}/5 (${h.count} entries)`
        ).join('\n')}`
      : ""
  ...
  const userMessage = `${langDirective}<note_to_enrich>${safeText}</note_to_enrich>${urlContext}${categoryContext}${forcedTypeContext}${globalContext}${confidenceHistoryContext}`
  ...
  if (result.confidence != null) {
    result.confidence = Math.min(5, Math.max(1, Math.round(result.confidence)))
  }
