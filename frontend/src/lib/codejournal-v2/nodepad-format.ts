/**
 * .codejournal file format — full-fidelity project serialisation.
 *
 * A .codejournal file is a JSON object with a version field so future schema
...
export const CODEJOURNAL_FILE_VERSION = 1 as const
...
export interface CodeJournalGhostNote {
  id: string
  text: string
  category: string
...
export interface CodeJournalFile {
  version: typeof CODEJOURNAL_FILE_VERSION
  exportedAt: number
  project: {
    id: string
    name: string
    ...
    collapsedIds: string[]
    ghostNotes: CodeJournalGhostNote[]
...
/** Serialise a project to a CodeJournalFile, stripping transient UI state. */
export function serialiseProject(project: {
  ...
  return {
    version: CODEJOURNAL_FILE_VERSION,
    exportedAt: Date.now(),
    project: {
      id: project.id,
      name: project.name,
      ...
      blocks: project.blocks.map((b: any): CodeJournalBlock => ({
        id:           b.id,
        text:         b.text,
...
/** Trigger a browser download of a .codejournal file for the given project. */
export function downloadNodepadFile(project: {
  ...
  const slug = project.name
    ...
    .slice(0, 60)
  a.href     = url
  a.download = `${slug || "project"}.codejournal`
  a.click()
...
export class CodeJournalParseError extends Error {}

/**
 * Parse a .codejournal file string and return a project object ready to push
...
export function parseNodepadFile(
  ...
  try {
    ...
    throw new CodeJournalParseError("Not a valid .codejournal file — JSON parse failed.")
  }

  if (!data || typeof data !== "object") {
    throw new CodeJournalParseError("Not a valid .codejournal file — unexpected root type.")
  }
  if (!data.project || !Array.isArray(data.project.blocks)) {
    throw new CodeJournalParseError("Not a valid .codejournal file — missing project.blocks array.")
  }
